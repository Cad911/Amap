...
(function() {
  ...
  Morris.Line = (function() {
    ...
    Line.prototype.drawPath = function(path, index, index_color) {
      var that,
        _this = this;
      that = this;
      that.r.path(path[index]).attr('stroke', that.options.lineColors[index_color]).attr('stroke-width', 0).animate({
        'stroke-width': 2
      }, 500, "bounce");
      true;
      ...
      if (index > 0) {
        setTimeout(function() {
          return _this.drawCircle(that.seriesCoords[index_color][index - 1].x, that.seriesCoords[index_color][index - 1].y, index_color, index - 1);
        }, 500);
      }
      if (index >= 0) {
        return setTimeout(function() {
          return _this.drawPath(path, index - 1, index_color);
        }, 500);
      } else {
        return true;
      }
    ...
    Line.prototype.drawSeries = function() {
      var c, circle, coords, i, path, _i, _ref, _results;
      this.seriesPoints = (function() {
        var _i, _ref, _results;
        _results = [];
        for (i = _i = 0, _ref = this.seriesCoords.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
          _results.push([]);
        }
        return _results;
      }).call(this);
      ...
      for (i = _i = _ref = this.seriesCoords.length - 1; _ref <= 0 ? _i <= 0 : _i >= 0; i = _ref <= 0 ? ++_i : --_i) {
        ...
        _results.push((function() {
          ...
          _ref1 = this.seriesCoords[i];
          _results1 = [];
          for (_j = 0, _len = _ref1.length; _j < _len; _j++) {
            c = _ref1[_j];
            if (c === null) {
              _results1.push(circle = null);
            } else {
    ...
    Line.prototype.createPath = function(all_coords, top, left, bottom, right) {
      var all_path, c, coords, g, grads, i, ix, lc, lg, path, x1, x2, y1, y2, _i, _j, _ref, _ref1;
      path = "";
      ...
      if (this.options.smooth) {
        ...
        all_path = [];
        for (i = _j = 0, _ref1 = coords.length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
          if (i === 0) {
            path = "M" + coords[i].x + "," + coords[i].y;
          } else {
            path = "M" + coords[i - 1].x + "," + coords[i - 1].y;
            path += "L" + (coords[i].x + 2) + "," + coords[i].y;
          }
          all_path.push(path);
        }
        return all_path;
    ...
    Line.prototype.drawHover = function() {
      ...
      this.hover = this.r.rect(-10, -this.hoverHeight / 2 - this.options.hoverPaddingY, 20, this.hoverHeight + this.options.hoverPaddingY * 2, 10).attr('fill', '#77614d').attr('stroke', '#ffffff').attr('stroke-width', 3).attr('opacity', this.options.hoverOpacity);
      this.xLabel = this.r.text(0, (this.options.hoverFontSize * 0.75) - this.hoverHeight / 2, '').attr('fill', '#ffffff').attr('font-weight', 'bold').attr('font-size', this.options.hoverFontSize);
