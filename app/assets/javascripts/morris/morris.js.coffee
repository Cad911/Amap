...
class Morris.Line
  ...
  drawPath: (path,index,index_color) ->
    that = this
    that.r.path(path[index])
       .attr('stroke', that.options.lineColors[index_color]) #@options.lineColors[i])
       .attr('stroke-width', 0)#options.lineWidth)
       .animate( {'stroke-width':2}, 500, "bounce")
    true

    
    if index > 0 # (path.length - 1)
      #ON DESSINE TOUS LES CERCLES
      setTimeout => 
        @drawCircle(that.seriesCoords[index_color][index-1].x,that.seriesCoords[index_color][index-1].y,index_color,index-1)
      ,500
    
    if index >= 0
      #ON DESSINE TOUS LES PATH
      setTimeout => 
        @drawPath(path,(index - 1),index_color)
      ,500
    else
       true
  
  drawCircle: (x,y,index_color,index_text) ->
    that = this
    circle = that.r.circle(x, y, that.options.pointSize)
            .attr('opacity',1)
            .attr('fill','#f7f4ec') #'#f7f4ec') #@options.lineColors[i])
            .attr('stroke-width', 2)
    ...
    for i in [@seriesCoords.length-1..0]
      ...
      for c in @seriesCoords[i]
        if c == null
          ...
          #circle = @drawCircle(c.x,c.y,i)
          #circle.attr('opacity',0)
        #@seriesPoints[i].push(circle)
      #console.log(@seriesCoords[i])
...
#         @seriesPoints[i].push(circle)
    ...
    if @options.smooth
      ...
      for i in [0..coords.length-1]
        ...
        if i is 0
          ...
          path = "C#{x1},#{y1},#{x2},#{y2},#{c.x},#{c.y}"#+= "C#{x1},#{y1},#{x2},#{y2},#{c.x},#{c.y}"
      ...
      for i in [0..coords.length-1]
          if i is 0
              path = "M#{coords[i].x},#{coords[i].y}"
          else
              path = "M#{coords[i-1].x},#{coords[i-1].y}"
              path += "L#{coords[i].x + 2},#{coords[i].y}" #ICI PLUS 2 CAR SINON CA APPARAIT DANS LE CERCLE
          all_path.push(path)
  ...
  drawHover: ->
    ...
    @hover = @r.rect(-10, -@hoverHeight / 2 - @options.hoverPaddingY, 20, @hoverHeight + @options.hoverPaddingY * 2, 10)
      .attr('fill','#77614d') #@options.hoverFillColor)
      .attr('stroke', '#ffffff')#@options.hoverBorderColor)
      .attr('stroke-width', 3)#@options.hoverBorderWidth)
      .attr('opacity', @options.hoverOpacity)
    @xLabel = @r.text(0, (@options.hoverFontSize * 0.75) - @hoverHeight / 2, '')
      .attr('fill', '#ffffff')#@options.hoverLabelColor)
      .attr('font-weight', 'bold')
      .attr('font-size', @options.hoverFontSize)
    @hoverSet = @r.set()
#     @rectTitle = @r.set()
#     @rectTitle.push(@rect_label)
#     @rectTitle.push(@xLabel)
    @hoverSet.push(@hover)
  ...
  updateHover: (index) =>
    ...
    #MEE
    #@rect_label.attr 'width', maxLabelWidth + @options.hoverPaddingX * 2
    #@rect_label.attr 'x', -@options.hoverPaddingX - maxLabelWidth / 2
    # move to y pos
